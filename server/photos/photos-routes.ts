...
export async function photosRoutes(fastify: FastifyInstance, photosService: PhotosService) {
  ...
  // PUT /photos/:id/image - upload image for existing photo
  fastify.put('/photos/:id/image', async (request, reply) => {
    try {
      const paramsResult = photoParamsSchema.safeParse(request.params);
      ...
      const photo = await photosService.uploadImage(
        id,
        buffer,
        data.filename
      );
